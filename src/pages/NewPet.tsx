...
const NewPetsManagementPage: React.FC = () => {
  ...
  const handleFinish = async (values: CreateAnimalForm) => {
    ...
    if (
      ...
      values.principalPictureUuid[0].originFileObj
    ) {
      const file = await uploadFile(
        values.principalPictureUuid[0].originFileObj
      );
      principalPicture = file.id;
    }
    let fileWithOrderList: AnimalListImage[] = [];
    if (values.imagesList && values.imagesList.length) {
      const fileWithOrderListWithUndefined = await Promise.all(
        values.imagesList.map(async (file, order) => {
          if (file.originFileObj) {
            const uploadedFile = await uploadFile(file.originFileObj); // Faz o upload e pega o arquivo
            if (uploadedFile) {
              return { imageUuid: uploadedFile.id, order }; // Retorna o objeto com id e order
            }
          }
        })
      );

      fileWithOrderList = fileWithOrderListWithUndefined
